...
export class CheUIElementsInjectorService {
   ...
   */
  addElementForInjection(parentElementId: string, additionalElementId: string, additionalElementHTML: string) {
   ...
   * @param parentElement: string - the parent DOM element for injection
   * @param jqAdditionalElement: ng.IAugmentedJQuery - the jqLite additional element
   * @param scope?: ng.IScope
   * @returns {boolean} - true if successful
   */
  injectAdditionalElement(parentElement: string | ng.IAugmentedJQuery, additionalElemen: string | ng.IAugmentedJQuery, scope?: ng.IScope) {
    if (!additionalElemen || !parentElement) {
      return false;
    }

    let jqAdditionalElement = angular.element(additionalElemen);
   ...
   * @returns {boolean} - true if successful
   */
  injectAdditionalElementByParentId(parentElementId: string, additionalElement: string | ng.IAugmentedJQuery) {
